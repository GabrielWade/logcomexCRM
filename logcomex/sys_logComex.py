...
new_table = pd.concat([new_table, matched_rows[crm_columns_to_take_adjusted + bi_columns_to_take_adjusted]])

# Para linhas que existem apenas na tabelaBI_agrupada
unmatched_rows = merged_data[merged_data['_merge'] == 'right_only']
new_table = pd.concat([new_table, unmatched_rows[bi_columns_to_take_adjusted]])
